...
if ($remaining_slots > 0) {
    ...
    // Se ainda faltar, pega de qualquer categoria
    $final_slots = $total_needed - count($recommendations_ids);
    if ($final_slots > 0) {
        $backup_query = new WP_Query(array(
            'post_type'      => 'post',
            'post_status'    => 'publish',
            'post__not_in'   => array_merge(array($post_id), $recommendations_ids),
            'posts_per_page' => $final_slots,
            'orderby'        => 'rand'
        ));
...
<section class="smart-recommendations mt-16 mb-12 py-12 border-t border-slate-100 dark:border-slate-800">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            ?>
            <a href="<?php the_permalink(); ?>" class="group relative bg-white dark:bg-slate-800 rounded-[32px] overflow-hidden border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col max-w-sm mx-auto w-full">
                
                <div class="aspect-video overflow-hidden relative">
                    <?php if (has_post_thumbnail()) : ?>
                        <?php the_post_thumbnail('medium_large', ['class' => 'w-full h-full object-cover transition-transform duration-700 group-hover:scale-110', 'loading' => 'lazy']); ?>
                    <?php endif; ?>
                ...
                <div class="p-6 flex-1 flex flex-col">
                    ...
                    <h4 class="text-base font-black text-slate-800 dark:text-white leading-tight mb-4 group-hover:text-primary transition-colors line-clamp-2">
                        <?php the_title(); ?>
                    </h4>
