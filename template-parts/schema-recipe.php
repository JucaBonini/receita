...
// 1. Coleta de Dados com Fallbacks Inteligentes (ACF & Novas Tabelas)
...
// Se não houver ingredientes cadastrados, delegamos para o schema-article.php para evitar erros de dados estruturados
if (empty($ingredientes_raw) || (is_array($ingredientes_raw) && empty(array_filter($ingredientes_raw)))) {
    include(get_template_directory() . '/template-parts/schema-article.php');
    return;
}
...
// 1. Organization
// URLs sociais do site gerenciadas via opções do WordPress (Customizer ou Options API)
$org_instagram = get_option('sts_social_instagram', 'https://www.instagram.com/descomplicandoreceitasofic');
$org_facebook  = get_option('sts_social_facebook', 'https://www.facebook.com/descomplicandoreceitasofic');
$org_youtube   = get_option('sts_social_youtube', 'https://www.youtube.com/@descomplicandoreceitas');
$org_tiktok    = get_option('sts_social_tiktok', 'https://www.tiktok.com/@desc_receitas_ofic');
$org_pinterest = get_option('sts_social_pinterest', 'https://www.pinterest.com/descomplicandoreceitas');
...
// 6. Recipe
$recipe_category = !empty($categories) ? $categories[0]->name : "Receita";
...
$recipe = [
    ...
    "author" => [
        ...
        "sameAs" => array_filter([
            get_option('sts_author_instagram', ''),
            get_option('sts_author_pinterest', ''),
        ]),
    ],
    "datePublished" => get_the_date('c', $post_id),
    "dateModified" => get_the_modified_date('c', $post_id),
...
// Otimização Sênior: Apenas adiciona NutritionInformation se houver calorias declaradas
if (!empty($calorias)) {
    $recipe["nutrition"] = [
        "@type" => "NutritionInformation",
        "calories" => (int)$calorias . " calories"
    ];
    if (!empty($carboidratos)) {
        $recipe["nutrition"]["carbohydrateContent"] = $carboidratos . " g";
    }
    if (!empty($proteinas)) {
        $recipe["nutrition"]["proteinContent"] = $proteinas . " g";
    }
    if (!empty($gorduras)) {
        $recipe["nutrition"]["fatContent"] = $gorduras . " g";
    }
}

// Otimização Sênior: Adiciona suitableForDiet se estiver preenchido
if (!empty($diet_type)) {
    $diet_map = [
        ...
        'sem glúten'      => 'https://schema.org/GlutenFreeDiet',
        'sem gluten'      => 'https://schema.org/GlutenFreeDiet',
        'low carb'        => 'https://schema.org/LowCarbDiet',
        'diabética'       => 'https://schema.org/DiabeticDiet',
        'diabetica'       => 'https://schema.org/DiabeticDiet',
        'halal'           => 'https://schema.org/HalalDiet',
        'kosher'          => 'https://schema.org/KosherDiet',
        'sem lactose'     => 'https://schema.org/LowLactoseDiet',
        // Nota: cetogênica/keto não possui equivalente em schema.org — omitir para evitar dados incorretos
    ];
    $diet_key = strtolower(trim($diet_type));
    if (isset($diet_map[$diet_key])) {
        $recipe["suitableForDiet"] = $diet_map[$diet_key];
    }
    // Não adicionamos um valor livre para evitar rejeição pelo validador do Google
}

// Otimização Sênior: Palavras-chave dinâmicas vindas das Tags do Post
...
if (!empty($post_tags) && !is_wp_error($post_tags)) {
    ...
    $recipe["keywords"] = implode(", ", $keyword_list);
}
