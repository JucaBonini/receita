...
function sts_sitemap_trigger() {
    ...
    if (preg_match('/sitemap-([a-z0-9\-]+)\.xml\/?$/i', $path, $matches)) {
        $requested_type = $matches[1];
        
        // Mapeamento de aliases e variações redundantes
        $aliases = [
        ...
        // 1. Se for um alias redundante, redireciona 301 para a URL canônica correta
        if (isset($aliases[$requested_type])) {
            $canonical_url = home_url("/sitemap-{$aliases[$requested_type]}.xml");
            wp_redirect($canonical_url, 301);
            exit;
        }

        // 2. Se a URL do sitemap terminar com barra (e.g. sitemap-post.xml/), redireciona 301 para sem barra
        if (substr($path, -1) === '/') {
            $canonical_url = home_url("/sitemap-{$requested_type}.xml");
            wp_redirect($canonical_url, 301);
            exit;
        }

        $type = $requested_type;
    } elseif (preg_match('/sitemap_index\.xml\/?$/i', $path)) {
        // 3. Se o índice de sitemaps terminar com barra, redireciona 301 para a versão limpa
        if (substr($path, -1) === '/') {
...
function sts_get_generic_sitemap_xml($post_type) {
    ...
    while ($query->have_posts()) {
        ...
        // ADIÇÃO ZEUS: Metadados Ricos de Imagem no Sitemap
        if (has_post_thumbnail($post_id)) {
            ...
            if ($thumb_url) {
                ...
                if (!empty($thumb_caption)) {
                    $xml .= '<image:caption>' . esc_xml($thumb_caption) . '</image:caption>';
                }
                $xml .= '</image:image>';
            }
        }
