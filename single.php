...
if (have_posts()) : while (have_posts()) : the_post();
    ...
    $conclusao    = get_post_meta($post_id, '_conclusao', true);
    ...
    <article>
        ...
        <?php if (!empty($video_url)) : ?>
            <div class="mb-12 aspect-video rounded-[32px] overflow-hidden shadow-2xl bg-black">
                <?php 
                $embed_url = strpos($video_url, 'youtube.com') !== false ? str_replace('watch?v=', 'embed/', $video_url) : $video_url;
                ?>
            ...
            <div class="lg:col-span-2">
                ...
                <div class="aeo-summary mb-10 p-6 sm:p-8 bg-primary/5 rounded-[32px] border border-primary/10 relative overflow-hidden group">
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <?php 
                        $dicas_mary = get_post_meta($post_id, '_informacoes_adicionais', true);
                        if (!empty($dicas_mary)) : 
                        ?>
                        <div class="bg-white/60 dark:bg-slate-900/40 p-5 rounded-2xl border border-primary/10">
                            <div class="flex items-center gap-2 mb-2 text-primary">
                                <span class="material-symbols-outlined text-sm">auto_awesome</span>
                                <span class="text-[10px] font-black uppercase tracking-widest">Segredo de Mestre</span>
                            </div>
                            <div class="text-sm sm:text-base text-slate-700 dark:text-slate-300 leading-relaxed prose-sm prose-slate">
                                <?php echo wp_kses_post($dicas_mary); ?>
                            </div>
                        </div>
                        <?php endif; ?>

                        <!-- 📊 SGE QUICK INSIGHTS: Tabela para o Google capturar como Resposta Direta -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                            <div class="flex items-center gap-3 p-3 bg-white/40 dark:bg-slate-800/20 rounded-xl border border-slate-100 dark:border-slate-800">
                                <span class="material-symbols-outlined text-primary text-xl">restaurant_menu</span>
                                <div>
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Culinária</p>
                                    <p class="text-xs font-bold text-slate-900 dark:text-white"><?php echo esc_html($cuisine_meta); ?></p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 p-3 bg-white/40 dark:bg-slate-800/20 rounded-xl border border-slate-100 dark:border-slate-800">
                                <span class="material-symbols-outlined text-primary text-xl">nutrition</span>
                                <div>
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Dieta</p>
                                    <p class="text-xs font-bold text-slate-900 dark:text-white"><?php echo esc_html($diet_type ?: 'Tradicional'); ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section class="mb-12" id="instructions">
                    ...
                    if (!empty($conclusao)) : 
                    ?>
                    <!-- Conclusão da Receita (Padrão de Excelência Google) -->
                    <div class="conclusao-receita mb-12 p-8 sm:p-10 bg-gradient-to-br from-primary/5 to-primary/0 dark:from-slate-800/30 dark:to-slate-800/10 rounded-[32px] border border-primary/10 relative overflow-hidden group">
                        <div class="absolute -bottom-6 -right-6 size-24 bg-primary/5 rounded-full blur-2xl group-hover:bg-primary/10 transition-all"></div>
                        <div class="flex items-start gap-4">
                            <div class="size-10 rounded-xl bg-primary text-white flex items-center justify-center shrink-0 shadow-lg shadow-primary/20">
                                <span class="material-symbols-outlined text-2xl font-light">emoji_objects</span>
                            </div>
                            <div>
                                <h3 class="text-xs font-black uppercase tracking-[0.2em] text-primary mb-3">Conclusão da Chef</h3>
                                <div class="text-slate-700 dark:text-slate-300 text-lg leading-relaxed prose prose-slate dark:prose-invert">
                                    <?php echo wp_kses_post($conclusao); ?>
                                </div>
                            </div>
                        </div>
                    </div>
