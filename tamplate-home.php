...
<main id="main-content">
    ...
    <!-- Campo de Pesquisa de Receitas (Leve, Fácil e Intuitivo) -->
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div class="bg-white dark:bg-slate-800 rounded-[24px] p-6 shadow-xl shadow-slate-100 dark:shadow-none border border-slate-100 dark:border-slate-700">
            <form role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>" class="flex flex-row gap-2 items-center w-full">
                <div class="relative flex-1">
                    ...
                    <input type="search" 
                           ...
                           class="w-full pl-10 pr-3 py-3.5 bg-slate-50 dark:bg-slate-900 border-0 focus:ring-2 focus:ring-primary rounded-2xl text-slate-700 dark:text-slate-200 placeholder-slate-400 font-medium text-sm sm:text-base outline-none transition-all duration-300">
                </div>
                <button type="submit" 
                        class="px-4 sm:px-6 py-3.5 bg-primary hover:bg-primary/90 text-white rounded-2xl font-black text-xs sm:text-sm transition-all duration-200 active:scale-95 shadow-md shadow-primary/20 shrink-0">
                    Buscar
                </button>
            </form>
            
            <!-- Sugestões de Buscas Populares -->
            <div class="flex flex-wrap items-center gap-2 mt-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
                <span class="text-slate-400">🔥 Buscas populares:</span>
                <a href="<?php echo esc_url(home_url('/?s=bolo+de+cenoura')); ?>" class="px-3 py-1.5 bg-slate-50 hover:bg-primary/10 dark:bg-slate-900 hover:text-primary dark:hover:text-primary rounded-full transition-all">Bolo de Cenoura</a>
                <a href="<?php echo esc_url(home_url('/?s=pernil')); ?>" class="px-3 py-1.5 bg-slate-50 hover:bg-primary/10 dark:bg-slate-900 hover:text-primary dark:hover:text-primary rounded-full transition-all">Pernil Assado</a>
                <a href="<?php echo esc_url(home_url('/?s=torta')); ?>" class="px-3 py-1.5 bg-slate-50 hover:bg-primary/10 dark:bg-slate-900 hover:text-primary dark:hover:text-primary rounded-full transition-all">Torta de Liquidificador</a>
                <a href="<?php echo esc_url(home_url('/?s=sobremesa')); ?>" class="px-3 py-1.5 bg-slate-50 hover:bg-primary/10 dark:bg-slate-900 hover:text-primary dark:hover:text-primary rounded-full transition-all">Sobremesas</a>
            </div>
        </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div class="@container">
            <div class="flex flex-col gap-8 lg:flex-row items-center lg:items-stretch">
                 ...
                 * Performance: Cacheada via transient para não bater no banco a cada page load.
                 */
                $candidates = get_transient('sts_hero_candidates');
                if (false === $candidates) {
                    $candidates = get_posts(array(
                        'post_type'      => 'post',
                        'posts_per_page' => 30, // Limite razoável — sem -1 ilimitado
                        'fields'         => 'ids',
                        'meta_key'       => 'destaque',
                        'meta_value'     => '1',
                    ));
                    ...
                    // Cache por 1 hora — atualiza automaticamente na próxima hora
                    set_transient('sts_hero_candidates', $candidates, HOUR_IN_SECONDS);
                }
